...
The essence of evolutionary analysis is to discuss how the probability changes when one side of the game changes. The learning ability of travelers, which is usually reflected by the tendency dynamic characteristics, in order to determine the change rate that is discussed in the next section.
...
\item payoff;
\item co-players: the set of agents that plays the game;
...
When the structure of travel mode choice reaches Nash equilibrium values in Figure \ref{f:32}, travel mode will remain stationary unless the payoffs of one or more mode parameters are changed. If a mode changes condition, a small perturbation appears. Then Nash equilibrium will be reached again after self adjusting.
\paragraph{}
...
\paragraph{}In the end, we ran several simulations (100 repetitions) and plot the average of car users and maximum values observed at every tick as shown in Figure 3.7, after increasing the travel cost by car.
\begin{figure}[!h]\label{f:aa}
\begin{center}
\includegraphics[width=12cm]{average cars avgmin}
\end{center}
\caption{Simulation 2: Average proportion of Car strategists}
\end{figure}
\clearpage
\paragraph{}The average proportions of public transport strategy are represented together with their minimum values in Figure 3.8.
\begin{figure}[h!]\label{f:bb}
\begin{center}
\includegraphics[width=12cm]{average prop avgmin}
\end{center}
\caption{Simulation 2: Average proportion of public transport strategists}
\end{figure}

\paragraph{}

In reality, traffic and weather conditions could affect the payoff of travel by increasing the time of travel. Travelers tend to choose some of the modes over other modes because of their relatively stable travel time in weather conditions.
